...
void mainGame::initScene(void)
{
	...
	SCENEMANAGER->addScene(L"결과씬", new sceneResult);
...
void mainGame::checkWindowSize(void)
{
	...
	else if (SCENEMANAGER->isCurScene(L"장비씬"))
	{
		setWindowResize({ WINSIZEX4,WINSIZEY4 });
	}
	else if (SCENEMANAGER->isCurScene(L"구매상점씬"))
	{
		setWindowResize({ WINSIZEX5,WINSIZEY5 });
	}
	else if (SCENEMANAGER->isCurScene(L"판매상점씬"))
	{
		setWindowResize({ WINSIZEX5,WINSIZEY5 });
	}
	else if (SCENEMANAGER->isCurScene(L"결과씬"))
	{
		setWindowResize({ WINSIZEX2,WINSIZEY2 });
